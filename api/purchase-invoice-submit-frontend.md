...
- `creditorCode`
  - 如果为空或无效，后端会自动改用账套里已有的 `CASH PURCHASE`
  - 注意：只允许 `CASH PURCHASE`，不会用 `CASH SALES`
  - 如果账套里不存在 `CASH PURCHASE`，submit 会失败
- `displayTerm`
  - 必填
  - 长度必须 `<= 30`
  - 必须是当前账套里已经存在的有效值
...
- `itemCode`
  - 如果有值，长度必须 `<= 30`
- `itemGroup`
  - 必填（服务端会先尝试按 `itemCode` 从本地 Item 主数据补齐；补不到就直接失败）
...
- `accNo`
  - 永远按 `details[].itemGroup -> ItemGroup.PurchaseCode` 补齐/覆盖
  - 如果 group 的 PurchaseCode 为空，则回退 `610-0000`
- `taxCode`
  - 优先按 `同供应商 + 同 itemCode` 的历史明细推最常用值
  - 推不到再按 `全局该 itemCode` 的历史推
  - 如果还是推不到，会继续保持空，不会再强制补默认税码
- `description`
  - 顶层如果为空，会自动补成 `PURCHASE INVOICE`（但如果前端传了别的值，会保留）
...
submit 阶段不会自动创建 creditor。

- 如果 `payload.creditorCode` 为空或无效，后端会自动改用账套里已有的 `CASH PURCHASE`。
- 如果账套里没有 `CASH PURCHASE`，submit 会失败（需要先在账套主数据建好）。
...
- 后端不会自动创建 `ItemGroup`
- 后端不会自动创建 `UOM`
- submit 阶段不会自动创建 `Creditor`（缺失时会用 `CASH PURCHASE` 兜底）
...
### `createMissing.items line X contains invalid base uom`

表示：

- 你提交的新 item 创建信息里
- `salesUom / purchaseUom / reportUom` 至少有一个不是当前账套可识别的 Base UOM


### `missing creditorCode` / `creditorCode does not exist`

表示：

- `payload.creditorCode` 为空或无效，且后端无法兜底到账套里的 `CASH PURCHASE`
- 通常原因是：账套主数据里不存在 `CASH PURCHASE`
